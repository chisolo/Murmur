...
public class ShopResearcherItem : MonoBehaviour
{
    ...
    public void Init()
    {
        _titleText.text = string.Format("<color=#1FFC92>{0}</color>/{1} {2}"
            , TalentModule.Instance.WorkTeam.Capacity
            , ConfigModule.Instance.Common().team_max
            , "RESEARCHERS".Locale());

        for (var i = 0; i < _teams.Count; i++) {
            var id = GameUtil.TalentTeamIdList[i];
            var has = TalentModule.Instance.WorkTeam.HasTeam(id);
            _teams[i].Init(id, has);
        }

        var isMAx = TalentModule.Instance.WorkTeam.IsTeamMax();
        _buyBtnRoot.gameObject.SetActive(!isMAx);
        _boughtRoot.gameObject.SetActive(isMAx);

        var teamId = TalentModule.Instance.FindNotOwnTeam();
        if (!string.IsNullOrEmpty(teamId)) {
            var productId = ShopModule.Instance.shopConfig.research_team_product_ids[teamId];
